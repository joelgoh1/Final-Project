...
from .workbench import CustomCard, custom_card_profiles, parse_custom_cards_form
...
class FixtureRequest(BaseModel):
    ...
    custom_cards: Optional[list[CustomCard]] = None
...
def _analyze_and_store(raw_rows, wallet_ids, source, source_meta, parse_quality=None, label=None, custom_cards=None) -> dict:
    result = analyze(
        raw_rows,
        ...
        card_overrides=custom_card_profiles(custom_cards),
    )
...
def bootstrap() -> dict:
    ...
    return {
        ...
        "categories": {k: v for k, v in labels.items() if k != "general_spend"},
        "cards": [
            {
                "id": card.id,
                "name": card.name,
...
async def analyze_upload(
    ...
    custom_cards: Optional[str] = Form(None),
) -> dict:
    ...
    return _analyze_and_store(
        ...
        quality,
        label=label,
        custom_cards=parse_custom_cards_form(custom_cards),
    )
